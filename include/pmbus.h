 ...
 * FilePath: \pmbusmonitor\include\pmbus.h
...
struct pmbus_data {
	...
	uint32_t func[PMBUS_PAGES];	/* Functionality, per page */
								/* Functionality bit mask 
								#define PMBUS_HAVE_VIN		(1 << 0)
								#define PMBUS_HAVE_VCAP		(1 << 1)
								#define PMBUS_HAVE_VOUT		(1 << 2)
								...
								#define PMBUS_HAVE_VMON		(1 << 18)
								#define PMBUS_HAVE_STATUS_VMON	(1 << 19)*/
	uint16_t sensor[PMBUS_PAGES][PSC_NUM_CLASSES];
	/*
	 ...
	 */
	int16_t (*read_byte_data)(struct i2c_client *client, uint8_t page, uint8_t reg);
	int32_t (*read_word_data)(struct i2c_client *client, uint8_t page, uint8_t reg);
	int (*write_word_data)(struct i2c_client *client, uint8_t page, uint8_t reg,
			       uint16_t word);
	int (*write_byte)(struct i2c_client *client, uint8_t page, uint8_t value);
	/*
	...
	void (*update_sensor)(struct i2c_client *client, struct pmbus_data *data);
	...
	// LIST_ENTRY(pmbus_data) list_entry;
};

extern void pmbus_scan(TwoWire *wire, uint8_t *address, uint8_t len, i2c_client_list_head *head);
extern float pmbus_linear2float(uint16_t data);
#endif /* PMBUS_H */
